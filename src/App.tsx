...
const copy = {
  es: {
    ...
    pipeline: {
      ...
      generatedPlanNotManual: "El Action Plan generado pertenece a CI/CD. Cambia a modo CI/CD Tool o genera/carga un Action Plan manual antes de continuar.",
      ...
      openRun: "Abrir run",
      steps: [
        "Validar el RFC, ambiente y alcance aprobado.",
        "Acceder al producto o instancia destino.",
        "Ejecutar los pasos del Action Plan aprobado.",
        "Validar que la configuracion o artefactos queden correctos.",
        "Capturar evidencia de ejecucion y validacion.",
        "Actualizar el RFC con el resultado y la evidencia."
      ]
    },
    evidence: {
      title: "Evidencia",
  ...
  en: {
    ...
    pipeline: {
      ...
      generatedPlanNotManual: "The generated Action Plan belongs to CI/CD. Switch to CI/CD Tool mode or generate/load a manual Action Plan before continuing.",
      ...
      openRun: "Open run",
      steps: [
        "Validate the RFC, environment, and approved scope.",
        "Access the target product or instance.",
        "Execute the approved Action Plan steps.",
        "Validate that the configuration or artifacts are correct.",
        "Capture execution and validation evidence.",
        "Update the RFC with the result and evidence."
      ]
    },
    evidence: {
      title: "Evidence",
  ...
  pt: {
    ...
    pipeline: {
      ...
      generatedPlanNotManual: "O Action Plan gerado pertence ao CI/CD. Altere para o modo CI/CD Tool ou gere/carregue um Action Plan manual antes de continuar.",
      ...
      openRun: "Abrir run",
      steps: [
        "Validar o RFC, ambiente e escopo aprovado.",
        "Acessar o produto ou instancia de destino.",
        "Executar os passos aprovados do Action Plan.",
        "Validar que a configuracao ou os artefatos estejam corretos.",
        "Capturar evidencia de execucao e validacao.",
        "Atualizar o RFC com o resultado e a evidencia."
      ]
    },
    evidence: {
      title: "Evidencia",
...
function splitActionPlanSections(text: string) {
  const lines = text
    ...
    .filter((line) => line && !/^=+$/.test(line) && !/^(?:Action Plan|End Action Plan)$/i.test(line));
...
export function App() {
  ...
  const currentPipelineSteps = useMemo(
    () => {
      ...
      return executionSteps.length ? executionSteps : parsedExecutionSteps;
    },
    [executionMode, executionSteps, parsedExecutionSteps]
  );
  ...
  function isWeakArtifactEntry(value: string) {
    ...
    if (/^<?artifact>?$/i.test(clean)) return true;
    if (/^<?(?:artifact|component|package|file)\s*\d*>?$/i.test(clean)) return true;
    return false;
  }
  ...
  function artifactTextCanBeFedFromInspection(value: string) {
    const lines = artifactLinesFromText(value);
    return !value.trim() || !lines.length || lines.every(isWeakArtifactEntry);
  }

  function repairedArtifactsForActionPlan(candidates: string[]) {
    const inspectedArtifacts = inspectedActionInstallableArtifacts();
    if (!inspectedArtifacts.length) return candidates;
    if (!candidates.length || candidates.every(isWeakArtifactEntry)) return inspectedArtifacts;
  ...
  function continuePendingWork(item: PendingWorkSnapshot) {
    ...
    if (draft) {
      ...
      const restoredPipelineActionPlan = wantsExecution && !draft.pipelineActionPlan.trim()
        ? draft.executionMode === "general" && actionPlanLooksLikeCicd(draft.actionPlan)
          ? ""
          : draft.actionPlan
  ...
  function generateActionPlan() {
      ...
      actionMethod === "manual" && !enteredArtifacts.length
        ? isMftManualPlan(effectiveProduct, sourceText)
          ...
          : sourceInstallableArtifacts.length
            ...
            : filterOicArtifacts(extractArtifactNames(sourceText, { includeComponentNames: true }))
        : [];
    const rawArtifacts = enteredArtifacts.length
    ...
    const componentNames = inspectedComponents.length
      ? inspectedComponents
      : artifacts.length
        ? artifacts.map((item) => artifactDisplayName(item))
  ...
  function actionPlanLooksLikeCicd(plan: string) {
    return /\bvia CI\/CD\b/i.test(plan) ||
      /\bCI\/CD confirmation\b/i.test(plan) ||
      /\bPrepare deployment package\b/i.test(plan) ||
      /\bRun pipeline\b/i.test(plan) ||
      /\bMerge Request\b/i.test(plan) ||
      /\bCI-CD Tool\b/i.test(plan);
  }

  function generatedActionPlanForManualExecution() {
    const sourceText = manualDetectionSourceText();
    const environment = effectiveActionEnvironment();
    const effectiveProduct = effectiveActionProductForText(sourceText);
    const currentSourceKey = manualPhaseSourceKeyFor(sourceText, environment, effectiveProduct);
    ...
    const plan = currentActionPlanForSupportOutput(sourceText, detectedPhases, disabledKeys);
    return actionPlanLooksLikeCicd(plan) ? "" : plan;
  }
  ...
  function useGeneratedActionPlanForExecution() {
    const generatedPlan = generatedActionPlanForManualExecution();
    if (!generatedPlan.trim()) {
      setMessage(t.pipeline.generatedPlanNotManual);
      return;
    }
    setPipelineActionPlan(generatedPlan);
    setExecutionSteps(parseActionPlanExecutionSteps(generatedPlan, t.pipeline.steps));
    setExecutionStepsConfirmed(false);
    setExecutionStepsReviewOpen(true);
    setPipelineStepIndex(0);
    setPipelineStepFailures({});
              ...
              <label className="execution-mode-field">
                ...
                <div className="segmented">
                  <button
                    ...
                    onClick={() => {
                      setExecutionMode("general");
                      if (pipelineActionPlan.trim() && !actionPlanLooksLikeCicd(pipelineActionPlan)) {
                        setExecutionSteps(parseActionPlanExecutionSteps(pipelineActionPlan, t.pipeline.steps));
                      } else {
                        setPipelineActionPlan("");
                        setExecutionSteps([]);
                      }
